...
BASE=/blue/ranka/sa.nandikanti/residual_runs/residual_prepare_all
OUT=/blue/ranka/sa.nandikanti/residual_runs/lbrc_cpp_validation_all
...
echo -e "dataset\tmethod\tfinal_nrmse\tcr\twall_s\tmaxrss_gib" > "$OUT/compare_summary.tsv"
...
npz = "$NPZ"
orig_path = Path("$ORIG")
rec_path = Path("$RECONS")
latent_path = Path("$LATENT")
copy_npz = Path("$SMALL_NPZ")
...
if not rec_path.exists():
    rec.tofile(rec_path)
latent_path.write_text(str(latent))
if not copy_npz.exists():
    np.savez(copy_npz, original_data=orig, recons_data=rec, latent_bit=np.array(latent, dtype=np.int64))

print("prepared", orig.shape, rec.shape, "latent_bit", latent)
PY

  LATENT_BIT=$(cat "$LATENT")

  echo "-- Python LBRC $DATASET --"
  cd "$LBRC_DIR"
  /usr/bin/time -v "$PY" -u LBRC.py \
    ...
    --quant_iter 16 \
    --workers 16 \
    > "$OUT/logs/${DATASET}_python.log" 2>&1
...
def parse(path):
    ...
    wall = w[-1] if w else "NA"
    rss = float(m[-1]) / 1024**2 if m else float("nan")
    return (n[-1] if n else "NA", c[-1] if c else "NA", wall, rss)

for method, log in [("python", "$OUT/logs/${DATASET}_python.log"), ("cpp", "$OUT/logs/${DATASET}_cpp.log")]:
    n, c, wall, rss = parse(log)
    print(f"{dataset}\\t{method}\\t{n}\\t{c}\\t{wall}\\t{rss:.3f}")
PY

  /usr/bin/grep -n "Target NRMSE\|Final NRMSE\|Encoded NRMSE\|CR:\|LBRC time\|Elapsed (wall clock)\|Maximum resident\|Traceback\|Error" \
    "$OUT/logs/${DATASET}_python.log" "$OUT/logs/${DATASET}_cpp.log" || true
done
