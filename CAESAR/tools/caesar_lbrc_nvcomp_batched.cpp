#include "models/lbrc_nvcomp_batched.h"
...
{
    std::cout
        << "CAESAR LBRC batched nvCOMP LZ4 residual evaluator\n\n"
        << "Usage:\n"
...
int main(int argc, char** argv)
{
    try
    {
        ...
        caesar::lbrc_nvcomp_batched::Options options;
        options.workers = 16;
        ...
        const caesar::lbrc_nvcomp_batched::Shape5D shape = caesar::lbrc_nvcomp_batched::parseShape5D(shapeText);
        const int64_t elements = caesar::lbrc_nvcomp_batched::numElements(shape);
        std::vector<float> original = caesar::lbrc_nvcomp_batched::loadFloat32File(originalPath, elements);
        std::vector<float> recons = caesar::lbrc_nvcomp_batched::loadFloat32File(reconsPath, elements);

        const auto start = std::chrono::steady_clock::now();
        const caesar::lbrc_nvcomp_batched::EvalResult result =
            caesar::lbrc_nvcomp_batched::evaluate(original, recons, shape, options);
