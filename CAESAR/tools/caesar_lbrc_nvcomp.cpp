...
{
    std::cout
        << "CAESAR LBRC nvCOMP LZ4 residual evaluator\n\n"
        << "Usage:\n"
...
int main(int argc, char** argv)
{
    try
    {
        ...
        caesar::lbrc_nvcomp::Options options;
        options.workers = 16;
        ...
        const caesar::lbrc_nvcomp::Shape5D shape = caesar::lbrc_nvcomp::parseShape5D(shapeText);
        const int64_t elements = caesar::lbrc_nvcomp::numElements(shape);
        std::vector<float> original = caesar::lbrc_nvcomp::loadFloat32File(originalPath, elements);
        std::vector<float> recons = caesar::lbrc_nvcomp::loadFloat32File(reconsPath, elements);

        const auto start = std::chrono::steady_clock::now();
        const caesar::lbrc_nvcomp::EvalResult result =
            caesar::lbrc_nvcomp::evaluate(original, recons, shape, options);
