...
{
    std::cout
        << "CAESAR LBRC residual evaluator\n\n"
        << "Usage:\n"
...
int main(int argc, char** argv)
{
    try
    {
        ...
        const caesar::lbrc::Shape5D shape = caesar::lbrc::parseShape5D(shapeText);
        const int64_t elements = caesar::lbrc::numElements(shape);
        std::vector<float> original = caesar::lbrc::loadFloat32File(originalPath, elements);
        std::vector<float> recons = caesar::lbrc::loadFloat32File(reconsPath, elements);

        const auto start = std::chrono::steady_clock::now();
        const caesar::lbrc::EvalResult result =
            caesar::lbrc::evaluate(original, recons, shape, options);
