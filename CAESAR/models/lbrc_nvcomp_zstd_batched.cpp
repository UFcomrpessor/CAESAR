#include "lbrc_nvcomp_zstd_batched.h"
...
namespace caesar
{
namespace lbrc_nvcomp_zstd_batched
{
namespace
{
...
void batchCompressBlocks(std::vector<Block>& blocks)
{
    ...
    size_t tempBytes = 0;
    checkNvcomp(nvcompBatchedZstdCompressGetTempSizeAsync(
                    chunks, maxInput, opts, &tempBytes, totalInput),
                "nvcompBatchedZstdCompressGetTempSizeAsync");

    size_t maxOut = 0;
    checkNvcomp(nvcompBatchedZstdCompressGetMaxOutputChunkSize(maxInput, opts, &maxOut),
    ...
    try
    {
        ...
        for (size_t i = 0; i < chunks; ++i)
        {
            checkNvcomp(statuses[i], "nvCOMP batched Zstd compression chunk status");
...
void batchDecompressBlocks(std::vector<Block>& blocks)
{
    ...
    size_t tempBytes = 0;
    checkNvcomp(nvcompBatchedZstdDecompressGetTempSizeAsync(
    ...
    try
    {
        ...
        for (size_t i = 0; i < chunks; ++i)
        {
            checkNvcomp(statuses[i], "nvCOMP batched Zstd decompression chunk status");
            if (actualBytes[i] != refs[i].unpacked)
                throw std::runtime_error("nvCOMP batched Zstd decompressed unexpected byte count");
            auto& dst = blocks[refs[i].block].streams[refs[i].stream];
...
} // namespace lbrc_nvcomp_zstd_batched_nvcomp_batched
} // namespace caesar
